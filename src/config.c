...
static int config_parse_vdev(int argc, char *argv[], void *data);
...
static struct config_keyword g_config_keywords[] = {
    ...
    {"vdev", config_parse_vdev, "DPDK virtual device args, eg net_tap0,iface=tap0,mac=aa:bb:cc:dd:ee:ff"},
...
//     if (cfg->vdev_num >= NETIF_PORT_MAX) {
//         printf("Error: too many vdev\n");
//         return -1;
//     }

//     port = &cfg->ports[cfg->port_num];
//     len = strlen(argv[1]);
//     if (len <= 0 || len >= RTE_ARG_LEN) {
//         printf("Error: bad vdev string\n");
//         return -1;
//     }
//     strcpy(cfg->vdev[cfg->vdev_num], argv[1]);
//     strcpy(port->dev_name, argv[1]);
...
static int config_check_port(struct config *cfg)
{
    ...
    if ((cfg->port_num == 0) && (cfg->vdev_num == 0)) {
        printf("no found port\n");
        return -1;
    }

    if (cfg->vdev_num > 0) {
        cfg->port_num = cfg->vdev_num;
    }
