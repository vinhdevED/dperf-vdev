...
static int dpdk_append_vdev(struct config *cfg, int argc, char *argv[])
{
    int i = 0;
    int num = 0;
    static char vdev_arg[NETIF_PORT_MAX][RTE_ARG_LEN + 8];

    for (i = 0; i < cfg->vdev_num; i++) {
        if (snprintf(vdev_arg[i], sizeof(vdev_arg[i]), "--vdev=%s", cfg->vdev[i]) >= (int)sizeof(vdev_arg[i])) {
            return -1;
        }
        argv[argc] = vdev_arg[i];
        argc += 1;
        num += 1;
    }
...
static int dpdk_eal_init(struct config *cfg, char *argv0)
{
    ...
    if (dpdk_append_vdev(cfg, argc, argv) < 0) {
        printf("dpdk append vdev fail\n");
        return -1;
    }
    argc += cfg->vdev_num;
